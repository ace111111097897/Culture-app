...
interface FunctionalKandiAIProps {
  username: string;
}

const FunctionalKandiAI: React.FC<FunctionalKandiAIProps> = ({ username }) => {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      text: `Hi ${username}! I'm Kandi, your AI companion. I can help you find compatible matches, suggest conversation starters, and provide cultural insights. What would you like to explore today?`,
      isUser: false,
      timestamp: new Date()
    }
  ]);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('kandi-ai-chat', {
        body: { 
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-3">
            <div className="bg-gradient-to-r from-teal-500 to-emerald-600 p-2 rounded-full">
            ...
            <Badge className="ml-auto bg-teal-100 text-teal-800">
              ...
              <div key={message.id} className={`flex ${message.isUser ? 'justify-end' : 'justify-start'}`}>
                <div className={`flex items-start space-x-2 max-w-xs lg:max-w-md ${message.isUser ? 'flex-row-reverse space-x-reverse' : ''}`}>
                  <Avatar className="h-8 w-8">
                    <AvatarFallback className={message.isUser ? 'bg-teal-500 text-white' : 'bg-emerald-500 text-white'}>
                      {message.isUser ? <User className="h-4 w-4" /> : <Bot className="h-4 w-4" />}
                    </AvatarFallback>
                  </Avatar>
                  <div className={`rounded-lg p-3 ${message.isUser ? 'bg-teal-500 text-white' : 'bg-emerald-50 text-emerald-800 border border-emerald-200'}`}>
                    <p className="text-sm">{message.text}</p>
                    <p className={`text-xs mt-1 ${message.isUser ? 'text-teal-100' : 'text-emerald-600'}`}>
              ...
              <div className="flex justify-start">
                <div className="flex items-start space-x-2">
                  ...
                  <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-3">
                    <p className="text-sm text-emerald-600">Kandi is thinking...</p>
                  </div>
                </div>
              </div>
            )}
          ...
          <div className="border-t p-4">
            <div className="flex space-x-2">
              <Input
                ...
                placeholder="Ask Kandi anything about dating and connections..."
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {suggestions.map((suggestion, index) => (
              <Button
                key={index}
                variant="outline"
                className="justify-start h-auto p-3 text-left border-teal-200 hover:bg-teal-50 hover:border-teal-400"
...
export default FunctionalKandiAI;